...
<!doctype html>
			...
			<div ng-repeat="employee in payroll_data.employee_info | orderBy:'id'" ng-if="employee.active">
				...
				<tr><th align="right">Hours Unpaid:</td><td class="hours">{{ payableLaborHours(employee.id, employee.project_payable_cutoff) - hoursPaid(employee.id, employee.project_payable_cutoff) | number : 2 }}</td></tr>
				...
				<tr><th align="right">Paid Vacation Hours:</td><td class="hours">{{ paidVacationHours(employee.id) | number : 2 }}</td><th align="right">Earned Vacation:</th><td class="hours">{{ vacationTally(employee.id, currenttimespanstart, currenttimespanend, 1) | number : 2 }}</td></tr>
				<tr><th align="right">Unpaid Vacation:</td><td class="hours">{{ loggedUnpaidVac(employee.id, employee.project_payable_cutoff) + hoursShort(employee.id, employee.project_payable_cutoff) - paidVacationHours(employee.id) | number : 2 }}</td><th align="right">Vacation Available:</th><td class="hours">{{ vacationTally(employee.id, beginningoftime, currenttimespanend) | number : 2 }}</td></tr>
